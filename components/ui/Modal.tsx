...
export function Modal({ isOpen, onClose, children, size = "lg", title }: ModalProps) {
  // Close on escape key
  useEffect(() => {
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className={`fixed bottom-6 right-6 px-4 py-3 rounded-xl shadow-lg flex items-center gap-3 z-[60] ${typeStyles[type]}`}
        >
          {icons[type]}
